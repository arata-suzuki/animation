...
void inputs_keyboard_parameters::update_from_glfw_key(int key, int action)
{
    ...
    if (key == GLFW_KEY_SPACE) {
        if (action_pressed) space = true;
        if (action_release) space = false;
    }

	if (key == GLFW_KEY_ENTER) {
        if (action_pressed) enter = true;
        if (action_release) enter = false;
    }
...
inputs_keyboard_parameters::inputs_keyboard_parameters()
	:shift(false), ctrl(false), up(false), down(false), right(false), left(false), space(false), enter(false)
{}
